...
# constants
MIN_GAP_DURATION_TO_COUNT = 1300 # 1.3 sec for large gap recognition
MAX_GAP_CURVE_TIME = 500         # time after which curve direction switches
BIAS_FINDING_TIME = 3500         
        ...
        while True:
            ...
            if left_value >= THRESHOLD and right_value >= THRESHOLD:
                ...
                if not in_gap and ticks_diff(ticks_ms(), last_white_detected) > 50:
                    gap_start_time = ticks_ms()                                # start time measurement gap
                    in_gap = True
                    gap_counted_already = False                                
                    uart.send("Gap found\n")
                if in_gap:         
                ...
                # bias determined --> changing gap crossing curve-direction
                if bias_completed:
                    if global_bias == "left":                           
                        if gap_duration < MAX_GAP_CURVE_TIME:
                            motors_obj.drive_soft_left(PWM_SPEED, 0.01)
                        else:                                            # runs out of time --> change curve-direction to find gap
                            motors_obj.drive_soft_right(PWM_SPEED, 0.01)
            ...
            if left_value >= THRESHOLD or right_value >= THRESHOLD: # reset in_gap when track found again
                in_gap = False
